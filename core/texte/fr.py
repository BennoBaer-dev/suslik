...
connues.</p>""",
    ...
    "gpu.keine_karte_cpu": "La reconnaissance tourne sur le processeur. Cela fonctionne, mais c'est environ six fois plus lent par analyse, et une deuxième place d'analyse n'y apporte rien : le processeur utilise déjà tous ses cœurs pour une seule.",
    "gpu.balken.speicher": "Mémoire",
    "gpu.balken.rechenzeit": "Temps de calcul",
    "gpu.worker": "place d'analyse",
    ...
    "gpu.noch_moeglich": "De la place pour {n} place(s) d'analyse de plus.",
    "gpu.passt_nicht": "Cela ne tient pas. Réduisez les veilleurs ou les places d'analyse, sinon le système atteint la limite mémoire et le noyau décide quel processus meurt.",
    "gpu.regler.titel": "Places d'analyse",
    "gpu.regler.erklaerung": "Combien d'événements peuvent être analysés en même temps. Sur {hw}, la valeur la plus rapide mesurée est {n}. Au-delà, nos tests n'ont pas été plus rapides.",
    ...
    "gpu.messwerte": "Mesuré sur nos machines : {worker} Mo par place d'analyse, {erster} Mo pour le premier veilleur direct (il porte les modèles) et environ {weiterer} Mo pour chaque suivant. Valeurs approximatives, pas une promesse.",
    "gpu.knopf": "GPU",
    ...
    "gpu.laeuft.zeile": "{plaetze} place(s) d'analyse, dont {belegt} occupée(s){klassen} · {waechter} veilleur(s) direct(s)",
    ...
    "gpu.noch_alt": 'Le service tourne encore avec {laeuft} place(s) ; {gesetzt} sont réglées, effectives au prochain redémarrage.',
    "gpu.passt_hoechstens": 'Avec les veilleurs actuels, au plus {n} place(s) tiennent en mémoire.',
    ...
    "gpu.laeuft_wie_eingestellt": 'Actif : le service tourne avec {n} place(s).',
    "gpu.vorschau_zu_wenig": 'Ne tient pas : il manque {fehlt}. Désactivez un veilleur direct ou retirez une place.',
    "gpu.speichern_gesperrt": 'Mémoire graphique insuffisante pour ce nombre de places.',
}
