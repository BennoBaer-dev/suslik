...
T = {
    ...
    "konfiguration.neustart.titel": "Restart service",
    "konfiguration.neustart.satz": "Restarts this service in place (the container keeps running, all data stays). Use it when processing seems stuck; open events are picked up again after the restart.",
    "konfiguration.neustart.knopf": "Restart now",
    ...
    "live.zaehler.funde": "{n} faces found",
    ...
    "live.erkennung.fenster_vor": "Decision window:",
    "live.erkennung.fenster_nach": "s — collect votes first, then announce the best name",
    "live.erkennung.fenster_hinweis": "From the first name candidate the watcher waits this long and then announces the candidate with the most votes — instead of the first one over the bar. 0 = announce immediately (behaviour up to 0.1.0.395).",
    ...
    "banner.nachholen.eins":
        "Catching up on missed events from the last hour: {gesamt} queued, "
        "{schlange} waiting, {arbeit} being analysed",
    "banner.nachholen.viele":
        "Catching up on missed events from the last {n} hours: {gesamt} queued, "
        "{schlange} waiting, {arbeit} being analysed",
    ...
    "antwort.unbek_weg": "This group no longer exists (merged or cleaned up in the meantime) — removing the card.",
...
between.</p>""",
    ...
    "systemstat.rueckstau.laeuft": "Backlog present",
    "systemstat.rueckstau.stand": "Queued · waiting · running",
    ...
    # ------------------------------------------------------ routes/gpu ---
    "gpu.titel": 'Graphics card',
    "gpu.speicher_gesamt": 'of graphics memory',
    "gpu.keine_karte": 'No graphics card in use.',
    "gpu.keine_karte_cpu": 'Recognition runs on the processor. That works, but it is roughly six times slower per analysis, and a second compute thread brings nothing there: the processor is already using all its cores for one.',
    "gpu.balken.speicher": 'Memory',
    "gpu.balken.rechenzeit": 'Compute time',
    "gpu.worker": 'compute thread',
    "gpu.waechter": 'live watcher',
    "gpu.dienst": 'service',
    "gpu.frei": 'free',
    "gpu.reserve": 'reserve',
    "gpu.belegt_andere": "in use, not attributable",
    "gpu.worker_prozess": "analysis worker (all threads)",
    "gpu.worker_unbekannt": "analysis worker (share not attributable)",
    "gpu.q.gemessen": "measured",
    "gpu.q.tabelle": "measurement table",
    "gpu.q.posten": "estimate",
    "gpu.q.config": "setting",
    "gpu.q.formel": "formula",
    "gpu.leiste_gemessen": "The bar shows the card as it is measured right now. Segments marked as an estimate come from our reference values, because this card does not report them per process.",
    "gpu.rechen.erklaerung": 'Live watchers decode their stream continuously, whether anyone is there or not. Event analysis runs in bursts and is not shown here.',
    "gpu.rechen.median": 'typical',
    "gpu.rechen.spitze": 'up to, on a busy camera',
    "gpu.rechen.bis": 'up to',
    "gpu.engpass.speicher": 'Memory is what limits you here.',
    "gpu.engpass.rechenzeit": 'Compute time is what limits you here, not memory.',
    "gpu.noch_moeglich": 'Room for {n} more compute thread(s).',
    "gpu.passt_nicht": 'This does not fit. Reduce watchers or compute threads, otherwise the system runs into the memory limit and the kernel decides which process dies.',
    "gpu.regler.titel": 'Compute threads',
    "gpu.regler.erklaerung": "How many events the analysis worker may compute at the same time. Background work — harvesting, collecting and anything you click on — runs beside them on its own thread and no longer takes an event's place.",
    "gpu.regler.legacy": 'Legacy mode without the analysis worker (worker: false) is switched on, and there this setting steers nothing: each event is computed in its own process, one after another.',
    "gpu.auto": 'automatic',
    "gpu.vorschlag_ist": 'currently {n}',
    "gpu.speichern": 'Save',
    "gpu.neustart_noetig": 'Takes effect after the next service restart.',
    "gpu.messwerte": 'Measured on our machines: {worker} MB per compute thread, {erster} MB for the first live watcher (it carries the models) and about {weiterer} MB for each further one. Approximate values, not a promise. Analyses running side by side were fastest on {hw} at {n}; more did not get faster in our tests.',
    "gpu.knopf": 'GPU',
    "gpu.knopf_tip": 'Graphics card: what it carries, and how to divide it between analysis and live watchers',
    "gpu.gespeichert": 'Saved. Takes effect after the next restart.',
    "gpu.nicht_messbar": "An accelerator is in use, but its memory cannot be read from inside the container. Recognition does run on it, only the memory bars below are missing. On Intel systems this is normal: the driver's counters are not readable there without extra rights.",
    "gpu.abschnitt.jetzt": 'Right now',
    "gpu.kachel.last": 'Card load',
    "gpu.kachel.last_unter": 'whole card, last hour',
    "gpu.kachel.last_eigen": 'Our share',
    "gpu.kachel.last_eigen_unter": "what suslik itself uses (the driver's total is not readable in a container)",
    "gpu.kachel.npu": 'NPU',
    "gpu.kachel.npu_unter": 'recognition runs here on Intel',
    "gpu.kachel.vram": 'Graphics memory',
    "gpu.kachel.vram_unter": 'in use on the card',
    "gpu.kachel.ram": 'Our memory',
    "gpu.kachel.ram_unter": 'an integrated GPU shares the system memory, it has none of its own',
    "gpu.kachel.temperatur": 'Temperature',
    "gpu.kachel.temperatur_unter": 'cards usually start slowing themselves down above 80 °C',
    "gpu.engine.render": 'render',
    "gpu.engine.compute": 'compute',
    "gpu.engine.video": 'video',
    "gpu.laeuft.titel": 'Running:',
    "gpu.laeuft.zeile": '{plaetze} compute thread(s), {belegt} of them busy{klassen} · {waechter} live watcher(s)',
    ...
    "gpu.klasse.live": 'body judgment (live)',
    "gpu.zu_live": 'Live watchers →',
    "gpu.noch_alt": 'The service is still running with {laeuft} thread(s); {gesetzt} is set and takes effect after the next restart.',
    "gpu.strang_ez": 'compute thread',
    "gpu.strang_mz": 'compute threads',
    "gpu.leiter_plaetze": 'The service runs {n} {strang} from the card budget; the setting is an upper limit.',
    "gpu.leiter_plaetze_wartet": 'The service is switching to {n} {strang} from the card budget; still in use until the next worker start: {jetzt} {strang_jetzt}. The setting is an upper limit.',
    "gpu.passt_hoechstens": 'With the current watchers, at most {n} thread(s) fit into the memory.',
    "gpu.vorschau": 'Would leave {rest} free.',
    "gpu.neustart_knopf": 'Restart service now',
    "gpu.neustart_frage": 'Restart the service now? Analyses in progress are stopped and picked up again afterwards.',
    "gpu.nicht_erreichbar": 'Service not reachable — it is probably restarting. Reload in a moment.',
    "gpu.laeuft_wie_eingestellt": 'Active: the service is computing on {n} thread(s).',
    "gpu.vorschau_zu_wenig": 'Does not fit — {fehlt} short. Switch off a live watcher or take a thread back.',
    "gpu.speichern_gesperrt": 'Not enough graphics memory for this number of threads.',
}
