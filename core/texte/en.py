...
between.</p>""",
    ...
    "gpu.keine_karte_cpu": 'Recognition runs on the processor. That works, but it is roughly six times slower per analysis, and a second analysis slot brings nothing there: the processor is already using all its cores for one.',
    "gpu.balken.speicher": 'Memory',
    "gpu.balken.rechenzeit": 'Compute time',
    "gpu.worker": 'analysis slot',
    ...
    "gpu.noch_moeglich": 'Room for {n} more analysis slot(s).',
    "gpu.passt_nicht": 'This does not fit. Reduce watchers or analysis slots, otherwise the system runs into the memory limit and the kernel decides which process dies.',
    "gpu.regler.titel": 'Analysis slots',
    "gpu.regler.erklaerung": 'How many events may be analysed at the same time. Measured fastest on {hw}: {n}. More did not get faster in our tests.',
    "gpu.auto": 'automatic',
    "gpu.vorschlag_ist": 'currently {n}',
    "gpu.speichern": 'Save',
    "gpu.neustart_noetig": 'Takes effect after the next service restart.',
    "gpu.messwerte": 'Measured on our machines: {worker} MB per analysis slot, {erster} MB for the first live watcher (it carries the models) and about {weiterer} MB for each further one. Approximate values, not a promise.',
    "gpu.knopf": 'GPU',
    ...
    "gpu.laeuft_wie_eingestellt": 'Active: the service is running with {n} slot(s).',
    "gpu.vorschau_zu_wenig": 'Does not fit — {fehlt} short. Switch off a live watcher or take a slot back.',
    "gpu.speichern_gesperrt": 'Not enough graphics memory for this number of slots.',
}
