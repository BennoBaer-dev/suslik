...
T = {
    ...
    "konfiguration.neustart.titel": "Dienst neu starten",
    "konfiguration.neustart.satz": "Startet diesen Dienst an Ort und Stelle neu (der Container laeuft weiter, alle Daten bleiben). Fuer den Fall, dass die Verarbeitung haengt; offene Events werden nach dem Neustart wieder aufgegriffen.",
    "konfiguration.neustart.knopf": "Jetzt neu starten",
    ...
    "live.zaehler.funde": "{n} Gesichter gefunden",
    ...
    "live.erkennung.fenster_vor": "Urteils-Fenster:",
    "live.erkennung.fenster_nach": "s — erst Stimmen sammeln, dann den besten Namen melden",
    "live.erkennung.fenster_hinweis": "Ab dem ersten Namens-Kandidaten wartet die Wache so lange und meldet dann den Kandidaten mit den meisten Stimmen — statt den ersten ueber der Latte. 0 = sofort melden (Verhalten bis 0.1.0.395).",
    ...
    "banner.nachholen.eins":
        ...
        "{gesamt} eingereiht, {schlange} in der Schlange, {arbeit} in Arbeit",
    ...
    "antwort.unbek_weg": "Diese Gruppe gibt es nicht mehr (inzwischen zusammengelegt oder aufgeräumt) — die Karte wird entfernt.",
...
Personen zwischendurch auffüllen.</p>""",
    ...
    "systemstat.rueckstau.laeuft": "Rückstand vorhanden",
    "systemstat.rueckstau.stand": "Eingereiht · in der Schlange · in Arbeit",
    ...
    # ------------------------------------------------------ routes/gpu ---
    "gpu.titel": 'Grafikkarte',
    "gpu.speicher_gesamt": 'Grafikspeicher',
    "gpu.keine_karte": 'Keine Grafikkarte im Einsatz.',
    "gpu.keine_karte_cpu": 'Die Erkennung läuft auf dem Prozessor. Das geht, ist je Analyse aber rund sechsmal langsamer, und ein zweiter Rechenstrang bringt dort nichts: der Prozessor nutzt für einen bereits alle Kerne.',
    "gpu.balken.speicher": 'Speicher',
    "gpu.balken.rechenzeit": 'Rechenzeit',
    "gpu.worker": 'Rechenstrang',
    "gpu.waechter": 'Live-Wächter',
    "gpu.dienst": 'Dienst',
    "gpu.frei": 'frei',
    "gpu.reserve": 'Reserve',
    "gpu.belegt_andere": "belegt, nicht zuordenbar",
    "gpu.worker_prozess": "Analyse-Worker (alle Stränge)",
    "gpu.worker_unbekannt": "Analyse-Worker (Anteil nicht zuordenbar)",
    "gpu.q.gemessen": "gemessen",
    "gpu.q.tabelle": "Messtabelle",
    "gpu.q.posten": "Richtwert",
    "gpu.q.config": "Einstellung",
    "gpu.q.formel": "Formel",
    "gpu.leiste_gemessen": "Der Balken zeigt die Karte so, wie sie gerade gemessen wird. Abschnitte mit „Richtwert“ stammen aus unseren Vergleichswerten, weil diese Karte sie nicht je Prozess meldet.",
    "gpu.rechen.erklaerung": 'Live-Wächter dekodieren ihren Stream durchgehend, ob jemand da ist oder nicht. Die Ereignis-Analyse rechnet in Schüben und steht hier nicht.',
    "gpu.rechen.median": 'üblich',
    "gpu.rechen.spitze": 'bis zu, an einer belebten Kamera',
    "gpu.rechen.bis": 'bis',
    "gpu.engpass.speicher": 'Hier begrenzt der Speicher.',
    "gpu.engpass.rechenzeit": 'Hier begrenzt die Rechenzeit, nicht der Speicher.',
    "gpu.noch_moeglich": 'Platz für {n} weitere Rechenstränge.',
    "gpu.passt_nicht": 'Das passt nicht. Nimm Wächter oder Rechenstränge zurück, sonst läuft das System in die Speichergrenze und der Kernel entscheidet, welcher Prozess stirbt.',
    "gpu.regler.titel": 'Rechenstränge',
    "gpu.regler.erklaerung": 'Wie viele Ereignisse der Analyse-Worker gleichzeitig rechnen darf. Hintergrund-Arbeit — Ernte, Sammeln und alles, was du anklickst — läuft daneben auf einem eigenen Strang und nimmt keinem Ereignis mehr seinen Platz weg.',
    "gpu.regler.legacy": 'Der Legacy-Modus ohne Analyse-Worker (worker: false) ist eingeschaltet, und dort steuert diese Einstellung nichts: jedes Ereignis rechnet in einem eigenen Prozess, eines nach dem anderen.',
    "gpu.auto": 'automatisch',
    "gpu.vorschlag_ist": 'derzeit {n}',
    "gpu.speichern": 'Speichern',
    "gpu.neustart_noetig": 'Wirkt ab dem nächsten Dienststart.',
    "gpu.messwerte": 'Auf unseren Maschinen gemessen: {worker} MB je Rechenstrang, {erster} MB für den ersten Live-Wächter (er trägt die Modelle) und rund {weiterer} MB für jeden weiteren. Näherungswerte, keine Zusage. Gleichzeitige Analysen waren auf {hw} mit {n} gemessen am schnellsten; mehr wurde in unseren Tests nicht schneller.',
    "gpu.knopf": 'GPU',
    "gpu.knopf_tip": 'Grafikkarte: was sie trägt und wie man sie zwischen Analyse und Live-Wächtern aufteilt',
    "gpu.gespeichert": 'Gespeichert. Wirkt ab dem nächsten Neustart.',
    "gpu.nicht_messbar": 'Ein Beschleuniger ist im Einsatz, aber sein Speicher lässt sich aus dem Container heraus nicht auslesen. Die Erkennung läuft sehr wohl darauf, es fehlen nur die Speicherbalken unten. Auf Intel-Systemen ist das normal: die Zähler des Treibers sind dort ohne zusätzliche Rechte nicht lesbar.',
    "gpu.abschnitt.jetzt": 'Gerade jetzt',
    "gpu.kachel.last": 'Auslastung',
    "gpu.kachel.last_unter": 'ganze Karte, letzte Stunde',
    "gpu.kachel.last_eigen": 'Unser Anteil',
    "gpu.kachel.last_eigen_unter": 'was suslik selbst nutzt (die Gesamtzahl des Treibers ist im Container nicht lesbar)',
    "gpu.kachel.npu": 'NPU',
    "gpu.kachel.npu_unter": 'auf Intel läuft hier die Erkennung',
    "gpu.kachel.vram": 'Grafikspeicher',
    "gpu.kachel.vram_unter": 'auf der Karte belegt',
    "gpu.kachel.ram": 'Unser Speicher',
    "gpu.kachel.ram_unter": 'eine integrierte GPU teilt sich den Arbeitsspeicher, sie hat keinen eigenen',
    "gpu.kachel.temperatur": 'Temperatur',
    "gpu.kachel.temperatur_unter": 'Karten bremsen sich meist erst oberhalb von 80 °C selbst',
    "gpu.engine.render": 'Render',
    "gpu.engine.compute": 'Compute',
    "gpu.engine.video": 'Video',
    "gpu.laeuft.titel": 'Läuft gerade:',
    "gpu.laeuft.zeile": '{plaetze} Rechenstränge, davon {belegt} belegt{klassen} · {waechter} Live-Wächter',
    ...
    "gpu.klasse.live": 'Körper-Urteil (live)',
    "gpu.zu_live": 'Zu den Live-Wächtern →',
    "gpu.noch_alt": 'Der Dienst läuft noch mit {laeuft} Strängen; eingestellt sind {gesetzt}, wirksam ab dem nächsten Neustart.',
    "gpu.strang_ez": 'Rechenstrang',
    "gpu.strang_mz": 'Rechenstränge',
    "gpu.leiter_plaetze": 'Der Dienst fährt {n} {strang} aus dem Kartenbudget; die Einstellung ist eine Obergrenze.',
    "gpu.leiter_plaetze_wartet": 'Der Dienst stellt auf {n} {strang} aus dem Kartenbudget um; bis zum nächsten Worker-Start weiterhin in Betrieb: {jetzt} {strang_jetzt}. Die Einstellung ist eine Obergrenze.',
    "gpu.passt_hoechstens": 'Mit den aktuellen Wächtern passen höchstens {n} Stränge in den Speicher.',
    "gpu.vorschau": 'Würde {rest} frei lassen.',
    "gpu.neustart_knopf": 'Dienst jetzt neu starten',
    "gpu.neustart_frage": 'Den Dienst jetzt neu starten? Laufende Analysen werden abgebrochen und danach erneut aufgenommen.',
    "gpu.nicht_erreichbar": 'Dienst nicht erreichbar — vermutlich startet er gerade neu. Gleich neu laden.',
    "gpu.laeuft_wie_eingestellt": 'Aktiv: der Dienst rechnet auf {n} Strängen.',
    "gpu.vorschau_zu_wenig": 'Passt nicht — {fehlt} zu wenig. Schalte einen Live-Wächter ab oder nimm einen Strang zurück.',
    "gpu.speichern_gesperrt": 'Zu wenig Grafikspeicher für diese Strangzahl.',
}
