...
T = {
    ...
    "konfiguration.neustart.titel": "Riavvia il servizio",
    "konfiguration.neustart.satz": "Riavvia questo servizio sul posto (il container resta attivo, tutti i dati rimangono). Da usare quando l'elaborazione sembra bloccata; gli eventi aperti vengono ripresi dopo il riavvio.",
    "konfiguration.neustart.knopf": "Riavvia adesso",
    ...
    "konfiguration.frigate_auth.titel": "Accesso a Frigate (facoltativo)",
    "konfiguration.frigate_auth.satz":
        "Serve solo se la vostra Frigate chiede di accedere \u2014 succede sulla sua "
        "porta autenticata (8971), non su quella interna (5000). Lasciando entrambi i "
        "campi vuoti non cambia nulla: suslik parla con Frigate esattamente come "
        "prima.",
    "konfiguration.frigate_auth.erkl_user":
        "nome utente di un account Frigate. Vuoto = nessun accesso; svuotandolo si "
        "cancella anche la password salvata",
    "konfiguration.frigate_auth.erkl_password":
        "password di quell'account. Viene salvata insieme alle altre impostazioni in "
        "/data e non viene pi\u00f9 mostrata \u2014 lasciate il campo vuoto per mantenerla",
    "konfiguration.frigate_auth.erkl_tls":
        "verificare il certificato TLS di Frigate. La porta autenticata di Frigate ha "
        "di fabbrica un certificato autofirmato: disattivate qui se la raggiungete via "
        "https e non avete sostituito quel certificato",
    "konfiguration.frigate_auth.pw_gesetzt": "salvata \u2014 lasciate vuoto per mantenerla",
    "konfiguration.frigate_auth.pw_leer": "nessuna password salvata",
    ...
    "live.test.veraltet_bitte":
        "Questo controllo ha {tage} giorni — rilancia il test della sorgente "
        "così i numeri descrivono quello che la telecamera dà oggi.",
    ...
    "live.zaehler.funde": "{n} volti trovati",
    ...
    "live.hoehe.alt_hinweis":
        "Salvato: {alt}p. Quel livello non c\u2019è più (misurato: nessun "
        "guadagno rispetto a {jetzt}p) — questa sentinella lavora a {jetzt}p. "
        "Il valore salvato e il test della sorgente restano validi; scegli "
        "uno dei livelli qui sopra per cambiare.",
    ...
    "live.detail.kein_bild": "Nessuna immagine dal vivo — questa sentinella non è in funzione.",
    "live.detail.kein_bild_test":
        "L\u2019ultimo test della sorgente ha visto {aufloesung}, elaborato come {skala}.",
    "live.detail.kein_bild_ohne_test":
        "Non è ancora stato fatto nessun test della sorgente, quindi di questo "
        "flusso non si sa niente.",
    "live.abschnitt.erkennung": "Riconoscimento",
    "live.abschnitt.melden": "Avvisi",
    "live.abschnitt.frigate": "Eventi Frigate",
    "live.abschnitt.erweitert": "Impostazioni avanzate",
    "live.abschnitt.abtastung": "Campionamento",
    "live.abtastung.schalter": "Guardare da vicino solo quando qualcosa si muove",
    "live.abtastung.erklaerung":
        "Il riconoscimento dei volti è la parte cara — un controllo di "
        "movimento economico sul piano di luminanza decide se un fotogramma "
        "vale la pena. Finché una persona viene seguita, la sentinella va "
        "sempre a ritmo pieno; solo una scena tranquilla viene campionata "
        "meno spesso.",
    "live.abtastung.ruhe_label": "Guardare comunque ogni",
    "live.abtastung.ruhe_einheit": "s",
    "live.abtastung.ruhe_hinweis":
        "Lascia vuoto per usare il tempo di fine comparsa di questa "
        "telecamera — chi sta fermo non crea movimento, quindi la sentinella "
        "dà un\u2019occhiata ogni tanto (1–600 s).",
    "live.abtastung.schwelle_label": "Sensibilità (variazione di grigio):",
    "live.abtastung.flaeche_label": "area minima:",
    "live.abtastung.eich_hinweis":
        "Lascia entrambi vuoti per i valori di fabbrica di Frigate. Numeri "
        "più bassi fanno guardare la sentinella più spesso; più alti la "
        "tengono tranquilla davanti a una siepe nel vento. Ogni posto è "
        "diverso — questi valori sono di QUESTA telecamera, non di tutto il "
        "sistema.",
    "live.abschnitt.guete": "Soglie di qualità dell\u2019immagine",
    "live.abschnitt.last": "Misura del carico",
    "live.erkennung.det_zeile":
        "Da un punteggio di rilevamento di {wert} in su una trovata conta "
        "come volto{marke}.",
    "live.erkennung.det_vorgabe": " (valore predefinito)",
    "live.erkennung.fenster_vor": "Finestra di giudizio:",
    "live.erkennung.fenster_nach": "s — prima raccogliere i voti, poi annunciare il nome migliore",
    "live.erkennung.fenster_hinweis": "Dal primo candidato la sentinella attende questo tempo e poi annuncia il candidato con piu voti, invece del primo sopra l'asticella. 0 = annuncio immediato (comportamento fino a 0.1.0.395).",
    "live.erkennung.regel_vor": "Riconosciuto dopo",
    "live.erkennung.regel_mitte": "conferme entro",
    "live.erkennung.regel_nach": "secondi",
    "live.erkennung.regel_hinweis":
        "0 secondi significa che conta l\u2019intera comparsa, come finora. "
        "Una finestra temporale aiuta sulle telecamere dove qualcuno resta "
        "fermo per minuti e due colpi fortunati molto distanti non dovrebbero "
        "valere come un riconoscimento.",
    "live.erkennung.vorrat_zeile":
        "Campioni di calibrazione: {n} di {deckel} \u2014 un\u2019immagine "
        "per comparsa, le più vecchie escono.",
    "live.erkennung.vorrat_aus":
        "La raccolta è spenta, quindi la pagina di calibrazione non ha nulla "
        "da mostrare (pagina di sistema, \"live_kalib_max\").",
    "live.erkennung.latte_e": "impressione dell\u2019immagine da {wert}",
    "live.erkennung.latte_t": "riconoscibilità da {wert}",
    "live.erkennung.latte_aus": "non impostato",
    "live.erkennung.latte_hinweis":
        "Questi due non decidono mai CHI viene riconosciuto: costerebbe "
        "conferme. Decidono quale immagine finisce nell\u2019avviso e quali "
        "volti restano come campioni di calibrazione. Si regolano nella "
    ...
    "live.frigate.erklaerung":
        ...
        "un proprio evento in Frigate con il nome nella sotto-etichetta "
        "\u2014 una registrazione propria, indipendente dal rilevamento di "
        "Frigate. La scrittura avviene in background: il guardiano non aspetta "
        "mai Frigate. In modalità sola lettura non viene scritto nulla.",
    "live.frigate.abstand_label": "Al massimo un evento per persona ogni (s):",
    "live.frigate.abstand_hinweis":
        "Vuoto significa: la stessa distanza impostata sopra per gli avvisi. "
        "Il limite vale per persona, quindi due persone diverse possono "
        "essere scritte insieme.",
    "livekalib.titel": "Calibrazione della telecamera — {name}",
    "livekalib.erklaerung":
        "Questi sono volti veri raccolti da questa telecamera \u2014 "
        "un\u2019immagine per comparsa. Sposta i cursori finché la selezione "
        "ti convince, poi applica. I valori valgono solo per QUESTA "
        "telecamera, perché le scale di qualità cambiano da telecamera a "
        "telecamera.",
    "livekalib.regler_det": "Punteggio di rilevamento",
    "livekalib.regler_det_prosa":
        "Da qui in su una trovata conta come volto. Questo cursore guida "
        "davvero il riconoscimento: tutto ciò che sta sotto non arriva mai "
        "alla verifica del nome. Più basso tiene più materiale (misurato: "
        "un\u2019asticella alta buttava via metà del materiale utile), più "
        "alto tiene fuori siepi e riflessi.",
    "livekalib.regler_e": "Impressione dell\u2019immagine",
    "livekalib.regler_e_prosa":
        "Quanto l\u2019immagine appare pulita e luminosa all\u2019occhio. "
        "Questo NON decide chi viene riconosciuto \u2014 decide quale "
        "immagine finisce nell\u2019avviso e quali volti restano qui come "
        "campioni.",
    "livekalib.regler_t": "Riconoscibilità",
    ...
    "livekalib.js.genutzt": "{n} campioni su {gesamt} passano",
    "livekalib.js.gespeichert": "salvato",
    "livekalib.js.fehler": "errore",
    # --- Kalibrier-Zentralumbau 31.08.: Abschnitte, Katalog-Latte, Material
    "livekalib.zur_uebersicht": "torna a tutte le telecamere",
    ...
    "lernwizard.balken.suchen": "Ricerca dei volti",
    "lernwizard.balken.pose": "Posa della testa",
    "lernwizard.balken.erkennen": "Riconoscimento dei volti",
    "lernwizard.balken.z_frames": "immagine {f} di {s}",
    "lernwizard.balken.z_posen.eins": "{n} posa",
    "lernwizard.balken.z_posen.viele": "{n} pose",
    "lernwizard.balken.z_erkannt.eins": "{n} riconosciuto",
    "lernwizard.balken.z_erkannt.viele": "{n} riconosciuti",
    "lernwizard.balken.wartet": "in attesa",
    "lernwizard.balken.clip": "recupero della clip …",
    ...
    "kalib.uebersicht.erklaerung": "Una telecamera, un insieme di valori. Le scale di qualità cambiano da telecamera a telecamera — misurato su un solo impianto, la riconoscibilità mediana di una telecamera era più del doppio di quella di un'altra. Scegli una telecamera per impostarne le soglie.",
    "kalib.uebersicht.leer": "Ancora nessuna telecamera",
    "kalib.uebersicht.leer_hinweis": "Appena Frigate segnala delle telecamere, compaiono qui.",
    "kalib.grenze.titel": "Cosa fa la calibrazione — e cosa no",
    "kalib.grenze.satz": "Decide quale immagine viene mostrata o inviata, quali volti restano come campioni e quali possono diventare un riferimento. Non decide mai chi viene riconosciuto: il controllo dei nomi vede ogni volto, senza filtri. È misurato, non supposto — filtrare prima del voto è costato conferme.",
    "kalib.kachel.eigene": "valori propri",
    ...
    "kalib.kachel.leer": "Ancora nessun campione",
    "kalib.kachel.leer_hinweis": "I campioni arrivano da un guardiano attivo e dalle analisi degli eventi — oppure prendine adesso.",
    ...
    "kalib.quelle.aus": "disattivata",
    "kalib.knopf_kalibrieren": "Calibra",
    ...
    "js.kalib.fehler": "ricerca di materiale non riuscita",
    ...
    "nav.system": "Sistema",
    "nav.systemstat": "Statistiche di sistema",
    ...
    "ui.live.chip": "Live",
    "ui.theme.knopf": "Tema",
    "ui.last.knopf": "Statistiche",
    "ui.last.tooltip": "Carico del sistema: CPU, RAM, disco, GPU e il riconoscimento",
    ...
    "js.unb.teil_frage": "Assegnare le {n} immagini selezionate a «{person}»? Il resto del gruppo resta in Sconosciuti.",
    ...
    "js.live.vorrat_leeren_frage":
        "Cancellare i campioni di calibrazione di questa telecamera? Non sono recuperabili; il guardiano ricomincia a raccoglierli da adesso.",
    ...
    "banner.nachholen.eins":
        "Recupero degli eventi persi nell’ultima ora: {gesamt} accodati, "
        "{schlange} in attesa, {arbeit} in analisi",
    "banner.nachholen.viele":
        "Recupero degli eventi persi nelle ultime {n} ore: {gesamt} accodati, "
        "{schlange} in attesa, {arbeit} in analisi",
    "banner.nachholen_aus": "In futuro non recuperare all’avvio",
    "hinweis.frigate_fr_an": "Il riconoscimento facciale di Frigate e acceso. suslik non ne ha bisogno: riconosce da solo e funziona in entrambi i casi. Puoi spegnerlo in Frigate se non lo usi per altro.",
    "ui.hinweis.x_tooltip": "Non mostrare piu questo avviso",
    "ui.hinweis.x_aria": "Nascondi definitivamente l'avviso",
    ...
    "unbekannte.mehr_bilder": "+{n} altre immagini in questo gruppo",
    "unbekannte.knopf_reaktivieren": "riattiva",
    "unbekannte.attr_name": "Nome (nuovo o esistente)",
    "unbekannte.attr_wahl": "Scegli questo gruppo per l'unione",
    "unbekannte.knopf_zuweisen": "Assegna persona",
    "unbekannte.knopf_teil": "Assegna le {n} selezionate",
    "unbekannte.knopf_ignorieren": "Ignora",
    "unbekannte.knopf_objekt": "Non è una persona",
    "unbekannte.knopf_person": "È una persona",
    "unbekannte.knopf_bulkmerge": "Unisci i {n} gruppi scelti",
    "unbekannte.knopf_mehr": "Mostra altri {n} gruppi",
    "unbekannte.titel": "Sconosciuti",
    "unbekannte.anker": "{offen} gruppi su {gesamt} sono ancora in attesa",
    "unbekannte.sort_label": "Ordine",
    "unbekannte.sort_bilder": "più immagini",
    "unbekannte.sort_neu": "più recenti",
    "unbekannte.filter_label": "Mostra",
    "unbekannte.f_offen": "da gestire",
    "unbekannte.f_wieder": "ricorrenti",
    "unbekannte.f_heute": "nuovi oggi",
    "unbekannte.f_vorschlag": "unione suggerita",
    "unbekannte.f_besucher": "silenziati",
    "unbekannte.f_objekt": "non persone",
    ...
    "unbekannte.satz_objekte":
        ...
        "in elenco, così nulla viene nascosto. Segnati a mano o trovati automaticamente; «È una persona» ne riporta uno indietro.",
    ...
    "unbekannte.leer_hinweis":
        ...
        "sconosciuto.",
    "unbekannte.leer_filter": "Niente in questa vista.",
    "unbekannte.leer_filter_hinweis": "Altri gruppi aspettano sopra, sotto un altro filtro.",
    ...
    "antwort.unbek_objekt": "segnato come non persona",
    "antwort.unbek_weg": "Questo gruppo non esiste più (nel frattempo unito o ripulito); la scheda viene rimossa.",
    "antwort.unbek_person": "di nuovo tra i visitatori",
    "antwort.unbek_gemergt": "{n} gruppi uniti in uno",
    ...
    "antwort.benennen_mismatch":
        "la tua selezione non corrisponde a nessuna immagine di questo "
        "gruppo — ricarica la pagina e rispunta le immagini",
    ...
    "baustein.kat.uebersprungen": "Saltato all’avvio",
...
vista del giorno integri le persone conosciute.</p>""",
    ...
    # --------------------------------------------- routes/systemstat ---
    "systemstat.titel": "Carico del sistema",
    "systemstat.sub":
        "Carico complessivo di questa macchina. Una nuova misura ogni "
        "{takt} secondi, conservate {stunden} ore. Qui non c\u2019è una "
        "ripartizione per processo: Frigate gira in un container proprio e "
    ...
    "systemstat.leer.hinweis":
        ...
        "del servizio. I valori che richiedono due misure (CPU, NPU, GPU) "
        "arrivano un giro più tardi.",
    "systemstat.block.hardware": "Hardware",
    "systemstat.block.erkennung": "Riconoscimento",
    "systemstat.block.live": "Live",
    "systemstat.nicht_verfuegbar": "non disponibile",
    "systemstat.kein_prozent": "nessuna percentuale",
    "systemstat.ja": "sì",
    "systemstat.nein": "no",
    "systemstat.verlauf.leer": "ancora nessuno storico",
    "systemstat.verlauf.aria": "l\u2019ultima ora",
    "systemstat.cpu.anzahl": "Core",
    "systemstat.cpu.kerne": "per core, adesso",
    "systemstat.kachel.platte": "Disco",
    "systemstat.ram.genutzt": "In uso",
    ...
    "systemstat.platte.frei_min": "Tenere libero",
    "systemstat.gpu.engine": "Motore più carico",
    "systemstat.gpu.speicher": "Memoria",
    "systemstat.gpu.temperatur": "Temperatura",
    "systemstat.gpu_eigen.titel": "GPU (quota di suslik)",
    "systemstat.gpu.gesamt": "Scheda intera",
    "systemstat.gpu_eigen.zeile": "quota di suslik",
    "systemstat.kachel.worker": "Processo di analisi",
    "systemstat.worker.laeuft": "in esecuzione",
    "systemstat.worker.ruht": "fermo, parte quando serve",
    "systemstat.worker.tode": "riavvii in 24 h",
    "systemstat.worker.zuletzt": "Ultima morte",
    "systemstat.worker.ursache": "Ultima causa",
    "systemstat.kachel.durchsatz": "Portata",
    "systemstat.durchsatz.tag": "Ultime 24 h",
    "systemstat.durchsatz.dauer": "Durata media",
    ...
    "systemstat.rueckstau.laeuft": "Arretrato presente",
    "systemstat.rueckstau.stand": "Accodati · in attesa · in analisi",
    "systemstat.rueckstau.stand_wert": "{gesamt} · {schlange} · {arbeit}",
    "systemstat.rueckstau.fenster": "Finestra retrospettiva",
    "systemstat.kachel.live": "Motore live",
    "systemstat.live.waechter": "Guardie attive",
    ...
    "systemstat.grund.kein_geraet":
        "in questa macchina non c\u2019è un dispositivo del genere",
    "systemstat.grund.kein_zaehler":
        "il dispositivo c\u2019è, ma il suo driver non pubblica alcun "
        "contatore di utilizzo",
    "systemstat.grund.gesperrt":
        "il contatore esiste, ma questo container non può leggerlo (gli "
        "eventi di prestazione del kernel richiedono permessi aggiuntivi)",
    "systemstat.grund.werkzeug_fehlt":
        "lo strumento di interrogazione di questo dispositivo non è in "
        "questa immagine",
    "systemstat.grund.nicht_lesbar": "questa fonte non si è potuta leggere",
    "systemstat.grund.kein_limit":
        "per questo container non è impostato alcun limite di memoria, "
    ...
    # ------------------------------------------------------ routes/gpu ---
    "gpu.titel": "Scheda grafica",
    "gpu.speicher_gesamt": "di memoria grafica",
    "gpu.keine_karte": "Nessuna scheda grafica in uso.",
    "gpu.keine_karte_cpu": "Il riconoscimento gira sul processore. Funziona, ma è circa sei volte più lento per analisi, e un secondo thread di calcolo lì non serve: il processore usa già tutti i suoi core per uno.",
    "gpu.balken.speicher": "Memoria",
    "gpu.balken.rechenzeit": "Tempo di calcolo",
    "gpu.worker": "thread di calcolo",
    "gpu.waechter": "sentinella live",
    "gpu.dienst": "servizio",
    "gpu.frei": "libero",
    "gpu.reserve": "riserva",
    "gpu.belegt_andere": "in uso, non attribuibile",
    "gpu.worker_prozess": "processo di analisi (tutti i thread)",
    "gpu.worker_unbekannt": "processo di analisi (quota non attribuibile)",
    "gpu.q.gemessen": "misurato",
    "gpu.q.tabelle": "tabella di misura",
    "gpu.q.posten": "valore indicativo",
    "gpu.q.config": "impostazione",
    "gpu.q.formel": "formula",
    "gpu.leiste_gemessen": "La barra mostra la scheda come viene misurata adesso. I segmenti con valore indicativo derivano dai nostri valori di riferimento, perché questa scheda non li segnala per processo.",
    "gpu.rechen.erklaerung": "Le sentinelle live decodificano il loro flusso di continuo, che ci sia qualcuno o no. L'analisi degli eventi lavora a ondate e non compare qui.",
    "gpu.rechen.median": "abituale",
    "gpu.rechen.spitze": "fino a, su una telecamera trafficata",
    "gpu.rechen.bis": "fino a",
    "gpu.engpass.speicher": "Qui il limite è la memoria.",
    "gpu.engpass.rechenzeit": "Qui il limite è il tempo di calcolo, non la memoria.",
    "gpu.noch_moeglich": "Spazio per altri {n} thread di calcolo.",
    "gpu.passt_nicht": "Così non ci sta. Riduci le sentinelle o i thread di calcolo, altrimenti il sistema raggiunge il limite di memoria e il kernel decide quale processo muore.",
    "gpu.regler.titel": "Thread di calcolo",
    "gpu.regler.erklaerung": "Quanti eventi il worker di analisi può calcolare contemporaneamente. Il lavoro in background — raccolta, prelievo e tutto ciò su cui fai clic — gira accanto su un thread proprio e non toglie più il posto a un evento.",
    "gpu.regler.legacy": "La modalità legacy senza worker di analisi (worker: false) è attiva, e lì questa impostazione non comanda nulla: ogni evento viene calcolato in un processo proprio, uno dopo l'altro.",
    "gpu.auto": "automatico",
    "gpu.vorschlag_ist": "attualmente {n}",
    "gpu.speichern": "Salva",
    "gpu.neustart_noetig": "Ha effetto dal prossimo riavvio del servizio.",
    "gpu.messwerte": "Misurato sulle nostre macchine: {worker} MB per thread di calcolo, {erster} MB per la prima sentinella live (porta i modelli) e circa {weiterer} MB per ogni ulteriore. Valori approssimativi, non una promessa. Le analisi in parallelo sono risultate più veloci su {hw} con {n}; di più non è risultato più veloce nelle nostre prove.",
    "gpu.knopf": "GPU",
    "gpu.knopf_tip": "Scheda grafica: cosa regge e come dividerla tra analisi e sentinelle live",
    "gpu.gespeichert": "Salvato. Ha effetto dal prossimo riavvio.",
    "gpu.nicht_messbar": "Un acceleratore è in uso, ma la sua memoria non è leggibile dall'interno del contenitore. Il riconoscimento gira eccome su di esso, mancano solo le barre di memoria qui sotto. Sui sistemi Intel è normale: i contatori del driver non sono leggibili senza permessi aggiuntivi.",
    "gpu.abschnitt.jetzt": 'Adesso',
    "gpu.kachel.last": 'Carico',
    "gpu.kachel.last_unter": 'intera scheda, ultima ora',
    "gpu.kachel.last_eigen": 'La nostra quota',
    "gpu.kachel.last_eigen_unter": 'quanto usa suslik (il totale del driver non è leggibile in un contenitore)',
    "gpu.kachel.npu": 'NPU',
    "gpu.kachel.npu_unter": 'su Intel il riconoscimento gira qui',
    "gpu.kachel.vram": 'Memoria grafica',
    "gpu.kachel.vram_unter": 'occupata sulla scheda',
    "gpu.kachel.ram": 'La nostra memoria',
    "gpu.kachel.ram_unter": 'una GPU integrata condivide la memoria di sistema, non ne ha una propria',
    "gpu.kachel.temperatur": 'Temperatura',
    "gpu.kachel.temperatur_unter": 'le schede di solito rallentano da sole sopra gli 80 °C',
    "gpu.engine.render": 'render',
    "gpu.engine.compute": 'calcolo',
    "gpu.engine.video": 'video',
    "gpu.laeuft.titel": 'In esecuzione:',
    "gpu.laeuft.zeile": '{plaetze} thread di calcolo, {belegt} occupati{klassen} · {waechter} sentinelle live',
    ...
    "gpu.klasse.live": 'giudizio corporeo (live)',
    "gpu.zu_live": 'Sentinelle live →',
    "gpu.noch_alt": 'Il servizio gira ancora con {laeuft} thread; ne sono impostati {gesetzt}, attivi dal prossimo riavvio.',
    "gpu.strang_ez": 'thread di calcolo',
    "gpu.strang_mz": 'thread di calcolo',
    "gpu.leiter_plaetze": 'Il servizio usa {n} {strang} dal budget della scheda; l\'impostazione è un limite superiore.',
    "gpu.leiter_plaetze_wartet": 'Il servizio passa a {n} {strang} dal budget della scheda; in uso fino al prossimo avvio: {jetzt} {strang_jetzt}. L\'impostazione è un limite superiore.',
    "gpu.passt_hoechstens": 'Con le sentinelle attuali entrano al massimo {n} thread nella memoria.',
    "gpu.vorschau": 'Lascerebbe liberi {rest}.',
    "gpu.neustart_knopf": 'Riavvia il servizio ora',
    "gpu.neustart_frage": 'Riavviare il servizio adesso? Le analisi in corso vengono interrotte e riprese dopo.',
    "gpu.nicht_erreichbar": 'Servizio non raggiungibile: probabilmente si sta riavviando. Ricarica tra poco.',
    "gpu.laeuft_wie_eingestellt": 'Attivo: il servizio calcola su {n} thread.',
    "gpu.vorschau_zu_wenig": 'Non ci sta: mancano {fehlt}. Disattiva una sentinella live o togli un thread.',
    "gpu.speichern_gesperrt": 'Memoria grafica insufficiente per questo numero di thread.',
}
